...
All commands are cross-platform: `./dev …` on Linux/macOS, `dev …` on Windows (same args).

## The two configs (and how they meet)

```
  characters.toml ──build──▶ roster.json ──▶ dev train ─────────┐
...
- **`il_graphs/characters.toml`** — *who* to train: identity tags, signature outfit, costume variants.
  Edited to add/remove a character; `./dev build il_graphs` turns it into `roster.json` + one
...
|---|---|---|
| `./dev build il_graphs` | Build all IL_* workflows from `characters.toml`; writes `roster.json`; **validates every emitted graph**. | (also `python tools/build_il_graphs.py`) |
| `./dev validate <graph>.json` | Pre-flight a workflow (models/wildcards/embeddings exist; rules: CLIP skip −2, CFG floor). | (also `python tools/validate_workflow.py`) |
| `./dev train <c>` | Caption + derive repeats + train ONE LoRA. | see the parameter matrix below; `--dry-run` |
| `./dev train --all` | `train` for every roster character with a dataset. | `--pack <name>`; extra flags pass through |
| `./dev models install il_graphs` | Download the Qwen-Image-Edit stack. | `--variant quant=Q4_K_M` |
| `python tools/lora_train/train_config.py --char <c>` | Print the resolved training params as JSON (what `dev train` will use). | `--profile`, `--set key=value` |
...
| `python tools/lora_train/cull_dataset.py <char>` | Flag blurry / near-duplicate frames (mechanical first-pass cull). Report-only by default. | `--apply`, `--blur`, `--dup`, `--no-blur`, `--no-dup` |
| `python tools/lora_train/gen_dataset.py <char> -n 40` | Headless: queue the `IL_DatasetEdit_<char>` graph N times with fresh seeds (needs ComfyUI running + a one-time **Export (API)**). | `-n/--count`, `--all`, `--seed`, `--url` |
| `<trainer-python> tools/lora_train/verify_env.py` | Verify the trainer venv (GPU bf16, sd-scripts, tagger, dataset TOML parse, wildcards). `<trainer-python>` = `tools/lora_train/.venv/bin/python` (Linux/macOS) or `…\.venv\Scripts\python.exe` (Windows). | — |

## Training parameter matrix

Precedence (**highest wins**): explicit **CLI flag** › **`--profile`** preset › **`[train.<char>]`** ›
**`[defaults]`**. Run `./dev train <c> --dry-run` to print the fully-resolved set + the exact
...
| `--d-coef` | `d_coef` | 1.0 | Prodigy effective-LR knob; 0.8 reduces overcook |
...
| `--min-images` | `min_images` | 12 | refuse to train on a smaller set |
| `--lr` | `lr` | `""` | learning_rate; empty = optimizer default (adamw/adafactor 3e-4; prodigy forced 1.0) |
| `--unet-lr` | `unet_lr` | `""` | empty = 3e-4 (adamw/adafactor) |
| `--text-encoder-lr` | `text_encoder_lr` | `""` | empty = 3e-5 (adamw/adafactor) |
| `--scheduler` | `lr_scheduler` | `cosine` | |
| `--min-snr` | `min_snr_gamma` | `5` | |
| `--resolution` | `resolution` | 1024 | dataset/bucket base resolution |
| `--bucket-min` | `min_bucket_reso` | 768 | |
| `--bucket-max` | `max_bucket_reso` | 1280 | |
| `--save-precision` | `save_precision` | `bf16` | |
| `--save-every-n-epochs` | `save_every_n_epochs` | 1 | |
| `--train-text-encoder` | `train_text_encoder` | false | also train the TE (drops `--network_train_unet_only`) |
...
are assembled in `devtools/train/cmd.py`.

**Non-param flags:** `<char>` (positional; dataset folder + LoRA name — `--char` also accepted),
`--pack` (which pack's `[train]` profile + roster; default `il_graphs`), `--all` (train every roster
character), `--trigger` / `--outfit` / `--prune` / `--keep` (default from `roster.json`), `--base`
(checkpoint override), `--skip-caption` (reuse captions), `--profile` (preset name), `--dry-run`
(preview only), `--force` (train even if the outfit baked nothing — see below).

### Outfit-bake guard

`dev train` aborts if an outfit/prune was given but **nothing** matched the captions — i.e. the outfit
...
field (or pass `--keep "coat, jacket"`); those tags are protected from pruning instead of baked. Use
WD14 vocab (same rule as `outfit`). Precedence at inference: baked `outfit` = permanent identity;
`keep` garments = optional layers.
...
```sh
./dev train nyx --profile complex              # use a preset
./dev train nyx --profile complex --steps 2400 # preset, one knob overridden
```
...
| The LoRA… | Dial |
|---|---|
| is weak / identity won't appear at inference | LoRA-bank strength ↑ (0.75 → 0.9); add `--train-text-encoder`; more `--steps` |
| is under-baked / low fidelity | `--dim 32 --alpha 16` (or `--profile complex`) — more capacity for complex characters |
| burns / overcooks | `--d-coef 0.8` (Prodigy) or `--optimizer adamw` (3e-4); fewer `--steps` |
...
| Symptom | Cause / fix |
|---|---|
| `dev train` "no dataset" | Generate to `output/dataset/<Char>/` first (SaveImage prefix `dataset/<Char>`). |
| "only N images (need ≥12)" | Generate/curate more, or lower `--min-images`. |
| "outfit matched NO caption tags" (abort) | Outfit-vocabulary mismatch — see the guard above; fix the `outfit` words, or `--force`. |
| kohya errors `sm_120 not supported` | Trainer torch isn't cu128 — re-run `./dev setup --with-trainer`. |
| `UnicodeEncodeError` (cp1252) during train | `PYTHONUTF8=1` (the tooling sets it; set it manually if you run sd-scripts directly). |
| OOM during training | drop `--batch 1`, keep `--network_train_unet_only` (don't disable the safety toggles). |
| A value "won't take effect" | something higher in the precedence chain overrides it — run `--dry-run` to see the resolved set. |
| Run died with no traceback | machine sleep / GPU TDR — disable sleep; per-epoch checkpoints survive, relaunch with `--skip-caption`. See [GOTCHAS.md](GOTCHAS.md). |
...
`./dev build il_graphs` validates every graph it writes:
...
- **Trainer = SDXL.** `dev train` runs kohya's `sdxl_train_network.py`, which trains a LoRA for **any
  SDXL checkpoint**. Point `--base` at another one:
  ```sh
  ./dev train aria --base models/checkpoints/someOtherSDXL.safetensors
  ```
  Resolution/bucketing/dim/alpha/optimizer are generic SDXL settings, not Illustrious-specific. (A pack
  can also set its own default `base_checkpoint` + `trainer_script` in its `pack.toml [train]` table.)
...
| **Realistic / photoreal SDXL** | ⚠️ runs, but | swap the captioner (WD14 → BLIP/natural language), edit the baked prompts + negative embeddings, and reconsider CLIP skip (−1) — change `clip_skip` in `build.py`'s rules + the `CLIPSetLastLayer` widgets. A dedicated realistic-character *pack* is the clean home for this (see `devtools/README.md`). |
| **SD 1.5 / Flux / SD3 / Qwen (as the trained model)** | ❌ | Different architecture: needs a different sd-scripts train script (`train_network.py` / `flux_train_network.py` / …) + params, declared in a pack's `[train]` profile. Not wired up. |
...
```sh
python -m pytest tools/tests -q          # or: uv run --with pytest pytest tools/tests -q
```
...
precedence chain; the cross-platform venv-exe resolution; the stdlib-only setup boundary; the
`accelerate`-command construction; and that this doc lists every flag + profile.
