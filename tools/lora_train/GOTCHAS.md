...
- **Machine sleep / GPU TDR kills training with no traceback.** A run died at step 566/1532 — all python
  ...
  Per-epoch checkpoints survive, so you lose at most one epoch; relaunch with `--skip-caption`.
...
- **Training defaults live in `train.toml`, not in code.** Don't expect `dim 16` / `steps 1500` hardcoded
  in `./dev train` — they resolve from `train.toml [defaults]`. **Precedence (highest
  wins): explicit CLI flag > `--profile` > `[train.<char>]` > `[defaults]`.** If a value "won't take
  effect," something higher in that chain is overriding it — run `./dev train <c> --dry-run` to
